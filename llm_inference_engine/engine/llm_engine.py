...
from llm_inference_engine.engine.metrics import EngineMetrics
...
class LLMEngine:
    ...
    def __init__(self, model, **kwargs):
        ...
        self.metrics = EngineMetrics(config.enable_metrics)
    ...
    def step(self):
        ...
        self.metrics.record_step(
            ...
            self.scheduler.last_metrics,
            self.model_runner.last_metrics,
        )
        for seq in seqs:
            if seq.is_finished:
                self.metrics.record_request(seq)
    ...
    def generate(
        ...
        for seq_id, token_ids in zip(sorted(outputs.keys()), sorted_outputs):
            ...
            self.metrics.record_detokenization(seq_id, (perf_counter() - detokenization_start) * 1000)
            decoded_outputs.append({"text": text, "token_ids": token_ids})
        self.metrics.finalize((perf_counter() - generation_start) * 1000)
